...
- **An `InputNumber`'s plus and minus flashed grey under the pointer.** Both
  washes a handle can wear were used in place of its background rather than
  over it, and both are mostly transparent — `handleActiveBg` is two per cent
  of the text colour. So a hover did not tint the button, it hollowed it: the
  white ground faded out over the hover animation and whatever stood behind
  the field showed through, then came back on the way out. A press did the
  same, harder. They are laid over `handleBg` now, and the two fields have
  probes of their own so the hole cannot come back.
...
- **`ThemeData.refineSeed` changes one thing about the seed and keeps the
  ...
  quietly dropped the font, the radii and the sizes the app had set. The new
  hook is handed whatever seed is in force, so nothing has to reach for a
  `BuildContext`:
